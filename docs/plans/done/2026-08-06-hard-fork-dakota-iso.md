...
**Executed 2026-09-15 under #841**, on explicit in-the-moment user confirmation ("Yes — reset main to 45cd7f22", backup ref requested, fork left under `kitten-lily/`). Results below.

- [ ] ~~Confirm `mise run build-iso` and `mise run iso-install-test`/`luks-install-test` have been green in real use … for a reasonable burn-in period.~~ **Not satisfied, waived by the user.** Phases 1-2 (#843) merged 2026-09-15 12:02 UTC and the demotion ran the same day. The port's own verification stands (PR #843 plus #840's own `mise run build-iso --debug` against a sibling checkout that no longer contains a single krytis file), but there was no burn-in. The pre-reset history is preserved on three refs, so the waiver is recoverable rather than irreversible.
- [x] Back up `kitten-lily/dakota-iso`'s current `main` before any reset — pushed `refs/heads/pre-demotion-backup` = `3d32f848` (all four commits). The two pre-merge branches `feat/sealed-payload-support` (`89bb5aa6`) and `feat/luks-passphrase-knob` (`a03f98db`) were left in place: neither is an ancestor of `3d32f848`, so they are independent history, not duplicates. Locally, `backup/pre-demotion-local` = `3d32f848` in the sibling checkout.
- [x] With explicit user confirmation: reset to the shared ancestor — `git push --force-with-lease=main:3d32f848… origin 45cd7f22…:refs/heads/main` (`+ 3d32f84...45cd7f2 … (forced update)`), then `gh repo sync kitten-lily/dakota-iso --source projectbluefin/dakota-iso --branch main` fast-forwarded it the remaining 11 commits to upstream's tip `f5fbf99`. Fork `main` and upstream `main` now report the identical SHA. The local checkout was repointed the way `docs/skills/upstream-sync.md` requires — `origin` = `projectbluefin/dakota-iso`, the fork demoted to a `fork` remote — and hard-reset to `origin/main`.
- [x] Add a `dakota-iso` entry to `docs/upstreams.yml` — `upstream: projectbluefin/dakota-iso`, `branch: main` (verified: the only long-lived branch upstream; 52 heads, every other one a topic branch), `local_path: dakota-iso`, `skill_file: docs/skills/dakota-iso.md`, `last_checked_sha: f5fbf99bbc843167522eec9078ede12308a16848`.
- [x] Create `docs/skills/dakota-iso.md` following `docs/skills/dakota.md`'s template shape — plus a `## Fork State` section recording where the four dropped commits live.
- [x] Confirm `mise upstream-sync dakota-iso` succeeds cleanly against the now-pure mirror: `==> dakota-iso (projectbluefin/dakota-iso, branch main)` / `Already up to date.` / `up to date at f5fbf99bbc84 — nothing new since the last lessons pass`, exit 0.
