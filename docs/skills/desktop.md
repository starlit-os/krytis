...
>
> **Retaining the splash means a dead greeter shows nothing at all**, so `config/greetd-config.bst` ships the other half: greetd is `Restart=always` with `StartLimitBurst=5` over 30s, and once it exhausts that it enters `failed` — at which point `OnFailure=krytis-greeter-fallback.service` runs `chvt 6`, forcing fbcon to repaint over the retained splash and landing on the rescue getty logind keeps on the reserved VT. Without it the box is unreachable: root is locked (`root:!unprovisioned`) and sshd ships disabled by preset.
>
