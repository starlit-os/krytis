...
Current locales: `sv_SE.UTF-8` and `en_SE.UTF-8`. `en_US.UTF-8` and `C.UTF-8` are already generated by `freedesktop-sdk.bst:components/utf-locale.bst` (which is part of `runtime-minimal.bst`). Do **not** re-generate them in `locale-data.bst` — BST will raise an overlap error at compose time.

`en_SE.UTF-8` was blocked for a long time and is not any more: `en_SE` arrived in glibc 2.43, and the fdsdk 26.08 bump (#305) took the junction to glibc 2.44. Added in #159.
...
mise run bst -- artifact checkout --directory /tmp/loc --deps none config/locale-data.bst
LOCPATH=/tmp/loc/usr/lib/locale LC_ALL=en_SE.UTF-8 locale -k LC_TIME LC_MONETARY
```

What the two locales should look like — `en_SE` is English *language* with Swedish *conventions*, which is the whole point of it:

| | `day` | `mon` | `d_fmt` | `currency_symbol` | `first_weekday` |
|---|---|---|---|---|---|
| `en_SE.UTF-8` | Sunday | January | `%Y-%m-%d` | kr | 2 (Mon) |
| `sv_SE.UTF-8` | söndag | januari | `%Y-%m-%d` | kr | 2 (Mon) |
| `en_US.UTF-8` | Sunday | January | `%m/%d/%Y` | $ | 1 (Sun) |
...
> **The splash-to-greeter handoff is `--retain-splash`, not ordering.** Read this before touching `plymouth-quit.service`.
>
> A previous version of this section claimed `plymouth-quit.service` has `After=display-manager.service`, and that greetd's `Alias=display-manager.service` therefore makes quit ordering automatic. That is false — fdsdk ships upstream plymouth's unit unmodified, whose only ordering is `After=rc-local.service plymouth-start.service systemd-user-sessions.service`. The `display-manager` edge is a *downstream* GDM addition in other distros. Verify with `systemctl cat plymouth-quit.service`, never from memory.
>
> The real ordering runs the other way: `greetd.service` is `After=plymouth-quit-wait.service`, so greetd is *guaranteed* not to start until the splash is already down. Measured on real hardware after #714: `plymouth-quit` finished at `18:00:37.0107`, the compositor's DRM init landed at `18:00:37.5633` — **0.55s with the display unowned**.
>
> Upstream's plain `plymouth quit` hands the framebuffer back to the text console for that window, and what shows through is machine-dependent — which is why a single-machine test is not enough here:
>
> |Machine|Underneath the splash|Observed|
> |---|---|---|
> |Desktop (AMD, DP)|empty tty1 — greetd's `Conflicts=getty@tty1.service` means no getty|black flash + cursor (`/sys/module/vt/parameters/global_cursor_default` is `1`)|
> |Laptop|EFI framebuffer leftovers|firmware BGRT logo + systemd-boot/systemd-stub version lines|
>
> `config/plymouth-theme.bst` therefore ships a drop-in making it `plymouth quit --retain-splash`, which leaves the last frame on the scanout until the compositor modesets — nothing underneath is revealed (#727). Note `vt.global_cursor_default=0` was considered and rejected: it suppresses the cursor only and does nothing for the laptop's firmware-logo reveal.
>
> **Retaining the splash means a dead greeter shows nothing at all**, so `config/greetd-config.bst` ships the other half: `OnFailure=krytis-greeter-fallback.service` on greetd, which runs `/usr/libexec/krytis/greeter-fallback.sh` and ends in `chvt 6` — forcing fbcon to repaint over the retained splash and landing on the rescue getty logind keeps on the reserved VT. Without it the box is unreachable: root is locked (`root:!unprovisioned`) and sshd ships disabled by preset.
>
> **`OnFailure=` fires on *every* entry into `failed`, not only when greetd gives up.** This is the trap that #733 fixed, and the reason the switch lives in a script rather than in `ExecStart=`. greetd is `Restart=always` with `RestartSec=1`, so an ordinary crash — the #712 class — is back within about two seconds; the first revision switched VTs unconditionally and so bounced the foreground to VT6 on every transient failure, mid-recovery, potentially dropping the user on a login console mid-password. Measured on hardware: five `systemctl kill -s KILL greetd.service` produced five `Triggering OnFailure= dependencies` and five VT switches, none of them warranted. `greeter-fallback.sh` therefore polls `systemctl is-active greetd.service` for ~5s and exits without touching the display if it recovers, short-circuiting immediately when `systemctl show -P Result greetd.service` reads `start-limit-hit` because no further restart is scheduled. `StartLimitBurst=5` permits five starts in the interval — the **sixth** trips it, so five kills land exactly on the limit without crossing it.
>
> **Do not "fix" the gap by adding `After=display-manager.service` to `plymouth-quit.service`.** greetd waits on `plymouth-quit-wait`, which runs `plymouth --wait` with `TimeoutSec=0` and blocks until plymouthd exits, which only happens when `plymouth-quit` runs — which would now be waiting on greetd. systemd sees **no cycle** (the `quit-wait → quit` link is a runtime dependency through plymouthd, not a declared edge), so it drops no edge and the boot hangs forever. GDM's own unit documents the trap: it inherits plymouth-quit's dependencies *"except for plymouth-quit-wait.service since it waits until plymouth is quit, which we do"*, takes `Conflicts=plymouth-quit.service`, quits plymouth itself, and keeps `OnFailure=plymouth-quit.service` as an escape hatch. Matching that shape would need greetd to issue the quit after its first frame, which means hooking upstream's `/usr/bin/noctalia-greeter-session` — an Upstream Gate conversation, and unnecessary now that `--retain-splash` covers the window.
>
> `mise run vt-owners-test` asserts all of this against the shipped image, including that nobody has re-added the deadlocking ordering.
...
**`Dialog`/`Title` alignment shares the same 0.0–1.0 canvas as `Watermark` — nothing auto-avoids overlap.** Stock spinner keeps the LUKS unlock prompt (`Dialog*Alignment`) and title text near vertical center (`.382`) because the stock watermark sits at the bottom (`.96`). Moving the watermark up to `.4` (this repo's change, see above) put it directly on top of the `.382` dialog/title — the two-step module does no collision detection, so overlapping widgets is silent until you actually look at the rendered splash. When repositioning any one of `Watermark`/`Dialog`/`Title`/spinner `VerticalAlignment`, re-check all four against each other, not just against the stock defaults. krytis currently uses: watermark `.4`, dialog/title `.7`, spinner `.85`. First attempt used `.6`/`.75` and still needed pushing further down — leave generous margin below the watermark's actual rendered height (503x375 px at `.4` center covers roughly `.4` ± (187/screen-height)), not just a nudge off the stock value.

**Color scheme is Rosé Pine Moon** (rose-pine.dev), not ad-hoc hex picks. `two-step`'s only color keys are `Background{Start,End}Color` and `ProgressBar{Background,Foreground}Color` — no dialog/text color keys exist to theme. Current mapping: background = Moon `base` (`#232136`), progress bar bg = Moon `highlightMed` (`#44415a`), progress bar fg = Moon `iris` (`#c4a7e7`). Reuse this palette for any other Plymouth color tweaks instead of picking new values.
