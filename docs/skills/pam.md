...
**Why a bracket control field, and not `sufficient`, for any module that can report "unavailable".** `auth sufficient pam_X.so` followed by `auth required pam_deny.so` collapses two distinct outcomes into one. `sufficient` treats every non-success as "ignore and continue", so `PAM_AUTHINFO_UNAVAIL` — "this authenticator is not usable right now" — reaches the application indistinguishable from a real `PAM_AUTH_ERR` once `pam_deny.so` closes the stack. dakota paid for this with fprintd (`ddc37d7`, its issue #674): a reader present but no prints enrolled returns `PAM_AUTHINFO_UNAVAIL`, GDM saw a generic failure instead of `ServiceUnavailable`, and GNOME Shell — which suppresses retries only for *unavailable* services — restarted `gdm-fingerprint` every few seconds, interrupting password entry at the greeter and lock screen. The fix is a control field that terminates the stack on that one result, so it survives as the return value:

```
auth  [success=done default=ignore authinfo_unavail=done]  pam_fprintd.so
```

A no-match still falls through to `pam_deny.so` and is rejected as before. Accepted tradeoff upstream: pam_fprintd reports its 30 s verification timeout as `PAM_AUTHINFO_UNAVAIL` too, so an enrolled user who idles past it gets a silent stop rather than a restart loop (`timeout=` is the knob).

**krytis has the identical shape — and must not copy the identical fix.** `elements/config/u2f-config.bst:83` and `:109` are `auth sufficient pam_u2f.so cue pinverification`, with `auth required pam_deny.so` closing both stacks (`:86`, `:112`); `authinfo_unavail` appears nowhere in the repo. Here the swallow is load-bearing. `pam_u2f` returns `PAM_AUTHINFO_UNAVAIL` whenever the per-user authfile cannot be opened — for a homed user on console login the home is still unmounted (§ *pam_u2f structurally cannot serve homed login* above; `u2f-config.bst:19-31` documents exactly this), and for *any* user with no key enrolled the file simply does not exist. `sufficient` ignoring that is what lets the stack fall through to `pam_systemd_home.so` and `pam_unix.so`. Adding `authinfo_unavail=done` would end the auth phase with `PAM_AUTHINFO_UNAVAIL` on every homed console login and on every `sudo` by an unenrolled account. What krytis forfeits by keeping `sufficient` is only the ability to tell a caller "no token present" apart from "wrong credential" — relevant if a krytis agent ever grows fprintd-style retry behaviour, not a reason to touch the stack now.

**Override mechanics, same commit: a separate element, never a junction patch.** dakota shipped the replacement `/etc/pam.d/fingerprint-auth` as its own `kind: manual` element that `depends` on `freedesktop-sdk.bst:components/linux-pam.bst` and whitelists the overlap, rather than patching the file through the junction — because the stock PAM file is a *source* of `linux-pam.bst`, so touching it there invalidates that element's cache key and cascades a rebuild of the whole runtime. Staging after the stock file wins the compose overlap deterministically. krytis already builds `u2f-config.bst` exactly this way (`depends: core/pam-u2f.bst`, `overlap-whitelist: /etc/pam.d/{system-auth,password-auth}` — `files/fakecap-manifest.tsv:551-552` shows both `components/linux-pam.bst` and `config/u2f-config.bst` claiming `/etc/pam.d/system-auth`); the rebuild-cascade reason is the part that was undocumented.
...
## `pam_securetty.so` blocks root login on the kmscon VTs — because kmscon logs in over a pty

fdsdk's `/etc/pam.d/login` (from `components/shadow.bst`, `files/fakecap-manifest.tsv:535`) opens with `auth required pam_securetty.so`. That module denies **root** on any terminal not named in `/etc/securetty`, and krytis inherits both halves from fdsdk — the module at `/usr/lib/x86_64-linux-gnu/security/pam_securetty.so` (`components/linux-pam-base.bst`, manifest `:167494`) and the list at `/etc/securetty` (`components/linux-pam.bst`, manifest `:598`). Verified on a live krytis box (`ID="krytis"`): `grep -l pam_securetty /etc/pam.d/* /usr/lib/pam.d/*` returns exactly `/etc/pam.d/login`, and `/etc/securetty` is six lines, `tty1`–`tty6`.

**The non-obvious part: the VT numbers are all listed, and root still cannot log in on vt2–4.** kmscon is a terminal emulator, not a getty — it allocates a pty (`objdump -T $(command -v kmscon)` shows `grantpt`/`ptsname_r`) and runs `/bin/login -p` on the slave side, so `login` sets `PAM_TTY` to `pts/N`. pam_securetty compares that string against `/etc/securetty`, which lists no pts devices, and returns `PAM_AUTH_ERR`. `tty2` being in the file is irrelevant; nothing on the kmscon console ever presents itself as `tty2`. Confirmed live: `systemctl list-units 'kmsconvt@*'` shows `kmsconvt@tty{2,3,4}.service` active, each `ExecStart=kmscon --vt=%I --no-switchvt` (`elements/config/kmscon.bst:58` enables exactly those three).

zirconium-hawaii hit this and stripped the module wholesale (`b248c79`, "Else logging in as root is broken. pam_securetty is also just plain obsolete, and most modern distributions disable it by default") — one `sed` in each OCI image element, across **both** PAM directories because pam 1.7 splits them, plus the same line removed from its own greetd stack:
...
**Two corrections to the obvious reading of krytis's exposure, both re-checked here.** First, the `ReserveVT` rescue console is *not* affected: `/etc/systemd/system/autovt@.service` symlinks to `getty@.service` (krytis deliberately never enables the bare `kmsconvt@` template, which would alias `autovt@` — see `elements/config/kmscon.bst:65-71`), so vt6 is a plain agetty running `login` directly on `/dev/tty6`, which *is* in `/etc/securetty`. The escape hatch `elements/config/greetd-config.bst:273-280` documents therefore still works. Second, root ships locked (`root:!unprovisioned`), so nothing is broken on a stock image. The bite is the case krytis's own docs tell you to create: an operator who sets a root password for rescue or VM debugging (`docs/skills/bootc-vm.md` § *Temporary root password for VM testing*) and then tries it on vt2–4 gets a bare authentication failure with no explanation — the password is correct and the login is refused.
