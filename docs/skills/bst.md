...
>
> Separate gap found while retracting it, deliberately left alone: the shipped `sshd_config` has no `Include /etc/ssh/sshd_config.d/*.conf`, so the drop-in that `/usr/lib/tmpfiles.d/20-systemd-userdb.conf` symlinks into place is inert (`sshd -T` → `authorizedkeyscommand none`). Adding the `Include` turns on systemd-userdb key lookup, i.e. changes authentication behaviour — Security Gate, not a packaging fix.
