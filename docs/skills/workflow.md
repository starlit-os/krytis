...
**File-editing tools (read/edit/write) resolve relative paths against the session's default cwd, not the worktree, unless told otherwise.** `git worktree add` does not change what a subsequent `edit`/`write` call by a repo-relative path (under `docs/skills/`, say) touches — the agent-facing edit tool has its own notion of "current directory" that is independent of any `cd`/`bash cwd` used to create the worktree, and it silently succeeds against whatever repo checkout that resolves to. Landed a skill-doc edit on `main` in the primary checkout instead of the intended worktree branch this way *twice in one session* (2026-09-09, PR #789) — the edit itself gave no error, `git status` in the worktree simply showed the file unmodified. Fix: always pass the **absolute worktree path** to the edit tool, never a bare repo-relative one, once a worktree exists. Verify with `git status --short -- <path>` **in the worktree** immediately after; if it shows clean, the edit went somewhere else — check the primary checkout and `git checkout -- <path>` to revert it before redoing the edit against the absolute worktree path.

**`git commit -m "<message>"` inside a double-quoted Bash-tool string still lets the shell expand backticks and `$(...)` in the message body.** A Conventional Commits body that references a command in backticks — e.g. `` Run `mise lock --upgrade` to migrate... `` — gets that snippet executed as a real command *during* the `git commit` invocation, not written literally; the actual command's stdout doesn't even end up in the commit message, and if the referenced command is expensive (an actual `mise lock`, a build, etc.) it just silently reruns it as a shell side effect. Never pass a message containing backticks via inline `-m "..."`. Write the message to a temp file and use `git commit -F <path>` instead — safe regardless of backticks, `$`, or quotes in the body.

**Never pass a multi-line PR/issue body to `gh` via a heredoc (`--body "$(cat <<'EOF' ... EOF)"`) in the `bash` tool.** Hit twice now, same failure shape both times (PR #779, 2026-09-08; PR #803, 2026-09-10): the heredoc's terminator match breaks in transport to the persistent shell, `gh pr create` itself never runs, and the shell then reads the **leftover, unconsumed body text as fresh top-level shell input**. Every line/word that happens to look like a real path or command gets executed for real. The #779 incident ran a first-boot wizard script and kicked off an unattended `mise run build --pull` on a nearly-full disk. The #803 incident triggered `mise run bst source track` with no element filter — a full 937-element project-wide tracking sweep — which rewrote the pinned `ref:` on 7 unrelated elements (caught via `git status`, reverted with `git checkout --`, before anything leaked into the pushed branch). Both times `gh pr list` came back empty and no `gh` process existed afterward — the PR itself was never created; only the leaked body text did anything. Fix, and the only fix: write the body with the `write` tool to a temp file, then `gh pr create --body-file <path>` (or `gh issue comment --body-file <path>`). Same root cause as the `git commit -m` backtick hazard directly above — any multi-line or quote-heavy content for a shell-invoked CLI goes through a temp file, never inline heredoc/quoting in the `bash` tool.
