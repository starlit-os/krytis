...
**Row 2 reaches polkit only because of a drop-in.** polkit ≥ 127 runs its PAM conversation inside a systemd sandbox that hides `/dev/hidraw*` and `/var/home` from `pam_u2f`, so the enrolled key is unreachable at a polkit prompt (and therefore for `run0`, `pkexec` and GUI privilege prompts) and the stack silently falls through to a password. `elements/config/polkit-agent-sandbox.bst` relaxes exactly those two options (#871) — see `docs/skills/pam.md` § polkit's sandboxed PAM helper. `sudo` never went through that helper and was unaffected.
